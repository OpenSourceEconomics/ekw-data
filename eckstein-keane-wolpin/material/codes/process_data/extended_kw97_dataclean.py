...
cond = df["CHOICE"].eq("home") & (
        (df["UNEMPLOYED"] + df["OUT_OF_LABOR_FORCE"] + df["NOT_WORKING"]) / df[
         "EMP_NONMISSING_WEEKS"]
).gt(1 / 3)
...
ext_kw_df.to_csv(PROJECT_DIR / "eckstein-keane-wolpin/eckstein-keane-wolpin-extended.csv",
                 index=False, sep="\t")
