...
class SourceCls(object):
    ...
    def _set_missing_values(self):
        ...
        MONTHLY_SCHOOL_ATTENDANCE = [
            col for col in source_long if col.startswith("ENROLLED_SCHOOL_")
        ]
    ...
    def testing(self):
        ...
        # positive.
        assert (
            source_long.filter(regex="EMP_HOURS_*")
            .apply(lambda column: (column[column.notnull()] >= 0).all())
            .all()
        )
        ...
        for varname in varnames:
            np.testing.assert_equal(
                (source_long[varname].notnull().groupby(level="Identifier").std() == 0).all(), True
            )
        ...
        values = (
            source_long.groupby(level="Identifier")
            .first()["ASVAB_ALTERED_TESTING"]
            .value_counts()
            .values
        )
        ...
        cases += [
            [(1988, source_long), (1356, 966, 415, 1825, 1016, 0, 1274, 617, 14, 113, 1271, 122)]
        ]
        ...
        cases = []
        cases += [
            [(1992, 14, source_long), (5697, 87, 194, 330, 762, 4009, 876, 379, 151, 74, 78, 0, 49)]
        ]
        cases += [
            [(2009, 7, source_long), (6760, 79, 144, 280, 664, 3350, 707, 375, 147, 83, 23, 33, 41)]
        ]
        cases += [
            [
                (2010, 52, source_long),
                (7170, 68, 111, 278, 558, 3061, 766, 356, 144, 68, 17, 44, 45),
            ]
        ]
...
def wide_to_long(source_wide, additional_level, dct):
    ...
    multi_index = pd.MultiIndex.from_product(
        [caseid, additional_level], names=["Identifier", "Survey Year"]
    )
