...
df['GRADE_COMPLETED'] = df['REAL_HIGHEST_GRADE_COMPLETED'].groupby(df['IDENTIFIER']
                                                                   ).diff().shift(-1)
...
gnp_deflation = pd.read_csv('../../sources/gnp_deflator_data/st_louis_fed_deflator.csv',
                            index_col='SURVEY_YEAR')

gnp_deflation['GNP_DEFL_BASE_1987'] = (gnp_deflation['IMPLICIT_GNP'] /
                                       gnp_deflation.loc[1987, 'IMPLICIT_GNP'])
...
categories_df.columns = ['OCC1990', 'DESCRIPTION', 'CPS_1970', 'CPS_2000_1', 'CPS_2000_5',
                         'category_service', 'CATEGORY', 'CPS_2002']
...
category_dict_1970 = dict(zip(categories_df.CPS_1970[cond_1].astype('Int64'),
                              categories_df.CATEGORY[cond_1]))

cond_2 = categories_df.CPS_2000_1.isin(range(1, 1000))
category_dict_2000 = dict(zip(categories_df.CPS_2000_1[cond_2].astype('Int64'),
                              categories_df.CATEGORY[cond_2]))

cond_3 = categories_df.CPS_2002.isin(range(1, 10000))
category_dict_2002 = dict(zip(categories_df.CPS_2002[cond_3].astype('Int64'),
                              categories_df.CATEGORY[cond_3]))
...
for week_num in [1, 7, 13, 14, 20, 26]:
    df['WHITE_COLLAR'] += (df['OCC_JC_WK_' + repr(week_num)] ==
                           'white_collar').astype(int).fillna(0)
    df['BLUE_COLLAR'] += (df['OCC_JC_WK_' + repr(week_num)] ==
                          'blue_collar').astype(int).fillna(0)
    df['MILITARY'] += (df['OCC_JC_WK_' + repr(week_num)] ==
                       'military').astype(int).fillna(0)
    df['UNEMPLOYED'] += (df['EMP_STATUS_WK_' + repr(week_num)].eq(4.0)).astype(int).fillna(0)
    df['OUT_OF_LABOR_FORCE'] += (df['EMP_STATUS_WK_' + repr(week_num)].
                                 eq(5.0)).astype(int).fillna(0)
...
df.loc[cond_1 & cond_2, 'CHOICE'] = (df.loc[cond_1 & cond_2,
                                            ['WHITE_COLLAR', 'BLUE_COLLAR', 'MILITARY']].
...
cond = (df['CHOICE'].eq('home') &
        ((df['UNEMPLOYED'] + df['OUT_OF_LABOR_FORCE'] + df['NOT_WORKING']) /
         df['EMP_NONMISSING_WEEKS']).gt(1 / 3))
...
cond = df['CHOICE'].eq('home')
df.loc[cond & work_cond_1 & ~work_cond_2 & work_cond_3 & work_cond_4,
       'HOME_CHOICE'] = 'part_time_work'
...
cond = (df['FAMILY_INCOME'].gt((0.5 * df['FAMILY_INCOME'].median())) &
        df['FAMILY_INCOME'].le(df['FAMILY_INCOME'].median()))
df.loc[cond, 'PARENTAL_INCOME_LOW'] = 1

df['PARENTAL_INCOME_HIGH'] = 0
cond = (df['FAMILY_INCOME'].ge(df['FAMILY_INCOME'].median()) &
        df['FAMILY_INCOME'].le((2 * df['FAMILY_INCOME'].median())))
...
cond = (df['AGE'].eq(16) & df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1).ge(10) &
...
cond = (df['AGE'].eq(16) & df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1).eq(9) &
        df['SURVEY_YEAR'].eq(1977) & df['CHOICE'].shift(-1).eq('schooling'))
df.loc[cond, 'CHOICE'] = df.loc[cond, 'SPECIFIC_CHOICE'] = 'schooling'
df.loc[cond, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[cond, 'aux_schooling'] - 1
...
df.loc[cond_1 & cond_2, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[cond_1 & cond_2,
                                                                 'aux_schooling'] - 1
...
cond = (df['REAL_HIGHEST_GRADE_COMPLETED'].isna() &
...
df['CONSECUTIVE_HIGHEST_GRADES_MISSING'] = df.groupby('IDENTIFIER'
