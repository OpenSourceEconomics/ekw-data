...
df = pd.merge(df, age_df, how='outer',
              left_on=['IDENTIFIER', 'AGE_OCT'], right_on=['IDENTIFIER', 'AGE'])
...
cond_2 = (~(df['REAL_HIGHEST_GRADE_COMPLETED'].shift(1).isna()) &
          (df['REAL_HIGHEST_GRADE_COMPLETED'].isna()) &
          ~(df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1).isna()))
cond_3 = ((df['REAL_HIGHEST_GRADE_COMPLETED'].shift(1) + 1) ==
          df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1))

df.loc[cond_1 & cond_2 & cond_3, 'REAL_HIGHEST_GRADE_COMPLETED'] = \
    df.loc[cond_1 & cond_2, 'AUX_HGC'] + 1
df.loc[~cond_1 & cond_2 & cond_3, 'REAL_HIGHEST_GRADE_COMPLETED'] = \
    df.loc[~cond_1 & cond_2, 'AUX_HGC']

df.drop(columns=['AUX_HGC'])

df['AUX_HGC'] = df['REAL_HIGHEST_GRADE_COMPLETED'].copy()

df['REAL_HIGHEST_GRADE_COMPLETED'] = df['REAL_HIGHEST_GRADE_COMPLETED'].\
    groupby(df['IDENTIFIER']).apply(lambda x: x.interpolate(method='linear', limit_area='inside'))
