...
def get_occ_hours(agent):
    ...
    for occ in ['white_collar', 'blue_collar']:
        ...
        for week_num in [1, 7, 13, 14, 20, 26]:
            ...
            agent[occ.upper() + '_HOURS'] += agent['EMP_HOURS_WK_' +
                                                   repr(week_num)].shift(-1).fillna(0).mul(cond)

        for week_num in [40, 46, 52]:
            cond = (agent['OCC_JC_WK_' + repr(week_num)] == occ).astype(int).fillna(0)
            agent[occ.upper() + '_HOURS'] += agent['EMP_HOURS_WK_' +
                                                   repr(week_num)].fillna(0).mul(cond)
...
def weeks_hours_worked(agent):
    ...
    for week_num in [40, 46, 52]:
        ...
        cond_3 = agent['EMP_STATUS_WK_' + repr(week_num)].ne(0) & ~agent['EMP_STATUS_WK_' +
                                                                         repr(week_num)].isna()
        agent['EMP_NONMISSING_WEEKS'] += cond_3.astype(int)

        agent['HOURS_LIST'] = (agent['HOURS_LIST'] +
                               agent['EMP_HOURS_WK_' + repr(week_num)].fillna(0).mul(cond_1)
                               .apply(lambda x: [x]))
    ...
    for week_num in [1, 7, 13, 14, 20, 26]:
        ...
        agent['HOURS_LIST'] = (agent['HOURS_LIST'] +
                               agent['EMP_HOURS_WK_' + repr(week_num)].shift(-1).fillna(0)
    ...
    for num in [1, 2, 3, 4, 5, 6]:
        ...
        agent.loc[cond, 'MAX_HOURS'] = agent.loc[cond, 'HOURS_LIST'].apply(lambda x:
                                                                           nlargest(num, x))
...
def clean_missing_wages(agent):
    ...
    for week_num in [40, 46, 52]:
        cond = agent['EMP_STATUS_WK_' + repr(week_num)].ge(100) & agent['WAGE_WK_' +
                                                                        repr(week_num)].isna()
        agent['MISSPECIFIED_EMP_STATUS'] += cond.astype(int)
...
def create_wages(agent):
    ...
    for week_num in [1, 7, 13, 14, 20, 26]:
        ...
        agent['REAL_SUM_WEEKLY_WAGES'] += ((nominal_weekly_wage /
                                            agent['GNP_DEFL_BASE_1987'].shift(-1)) * cond).fillna(0)
        agent['WEEKS_IN_MAJOR_OCC'] += cond
        agent['HOURLY_WAGES'] += (agent['WAGE_WK_' + repr(week_num)].shift(-1) /
                                  agent['GNP_DEFL_BASE_1987'].shift(-1))

    for week_num in [40, 46, 52]:
        cond = (agent['OCC_JC_WK_' + repr(week_num)] == agent['CHOICE']).astype(int)
        nominal_weekly_wage = (agent['WAGE_WK_' + repr(week_num)] *
                               agent['EMP_HOURS_WK_' + repr(week_num)])
        agent['NOMINAL_SUM_WEEKLY_WAGES'] += (nominal_weekly_wage * cond).fillna(0)
        agent['REAL_SUM_WEEKLY_WAGES'] += ((nominal_weekly_wage /
                                            agent['GNP_DEFL_BASE_1987']) * cond).fillna(0)
        agent['WEEKS_IN_MAJOR_OCC'] += cond
    ...
    agent.drop(columns=['REAL_SUM_WEEKLY_WAGES', 'NOMINAL_SUM_WEEKLY_WAGES',
                        'REAL_AVG_WEEKLY_WAGE', 'NOMINAL_AVG_WEEKLY_WAGE', 'WEEKS_IN_MAJOR_OCC',
                        'HOURLY_WAGES'], inplace=True)
...
def create_military_wages(agent):
    ...
    for week_num in range(1, 53):
        agent['WEEKS_MILITARY_PER_CALENDAR_YEAR'] += (agent['EMP_STATUS_WK_' +
                                                            repr(week_num)].eq(7.0)).astype('int')

    agent['WEEKLY_MILITARY_INCOME'] = (agent['INCOME_MILITARY'] /
                                       agent['WEEKS_MILITARY_PER_CALENDAR_YEAR'])

    for week_num in range(1, 40):
        cond = (agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).eq(7.0)).astype(int)
        agent['sum_military_income'] += (agent['WEEKLY_MILITARY_INCOME'].shift(-1) /
                                         agent['GNP_DEFL_BASE_1987'].shift(-1)).fillna(0) * cond
        agent['weeks_in_military'] += cond

    for week_num in range(40, 53):
        cond = (agent['EMP_STATUS_WK_' + repr(week_num)].eq(7.0)).astype(int)
        agent['sum_military_income'] += (agent['WEEKLY_MILITARY_INCOME'] /
                                         agent['GNP_DEFL_BASE_1987']).fillna(0) * cond
        agent['weeks_in_military'] += cond

    agent['CALCULATED_MILITARY_INCOME'] = (agent['sum_military_income'] /
                                           agent['weeks_in_military']) * 50
