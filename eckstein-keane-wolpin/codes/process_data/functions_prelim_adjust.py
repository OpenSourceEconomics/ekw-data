...
def extend_survey_year(agent):
    ...
    agent.loc[cond, 'SURVEY_YEAR'] = (agent['aux_extend_survey_year'] +
                                      agent['AGE']).astype('Int64')
...
def simple_two_grade_jump(agent):
    ...
    cond_2 = agent['REAL_HIGHEST_GRADE_COMPLETED'] \
        .eq(agent['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1))
    cond_3 = (agent['MONTHS_ATTENDED_SCHOOL'].shift(1).gt(0) &
              agent['MONTHS_ATTENDED_SCHOOL'].gt(0))
...
def data_shift(agent):
    ...
    for week_num in [1, 7, 13, 14, 20, 26, 40, 46, 52]:
        for num in [1, 2, 3, 4, 5]:
            ...
            agent_ext.drop(['JOB_' + repr(int(num)) + '_WK_' + repr(week_num)], axis=1,
                           inplace=True)
            agent_ext.drop(['WAGE_' + repr(int(num)) + '_WK_' + repr(week_num)], axis=1,
                           inplace=True)
