...
TIME_VARYING = []
TIME_VARYING += ['REVISED_ENROLLMENT_STATUS_MAY', 'UNREVISED_ENROLLMENT_STATUS_MAY']
TIME_VARYING += ['YEAR_OF_BIRTH', 'CPSOCC70', 'MONTH_OF_BIRTH']
TIME_VARYING += ['REVISED_HIGHEST_GRADE_COMPLETED_MAY', 'UNREVISED_HIGHEST_GRADE_COMPLETED_MAY']
...
class SourceCls(object):
    ...
    def _set_missing_values(self):
        ...
        MONTHLY_SCHOOL_ATTENDANCE = [col for col in source_long
                                     if col.startswith('ENROLLED_SCHOOL_')]
    ...
    def testing(self):
        ...
        # positive.
        assert source_long.filter(regex='EMP_HOURS_*')\
            .apply(lambda column: (column[column.notnull()] >= 0).all()).all()
        ...
        values = source_long.groupby(level='Identifier').first()['ASVAB_ALTERED_TESTING']\
            .value_counts().values
        ...
        for year in [1979, 1993]:
            cond = source_long['CPSOCC70'][:, year].equals(
                source_long['OCCALL70_MOD_JOB_1'][:, year])
        ...
        cases += [[(1988, source_long), (1356, 966, 415, 1825, 1016, 0, 1274,
                                         617, 14, 113, 1271, 122)]]
        cases += [[(1993, source_long), (1276, 909, 313, 1393,  857, 1,
                                         942, 557, 24, 47, 1171, 60)]]
        ...
        # EMP_HOURS
        cases = []
        cases += [[(1992, 14, source_long), (5697, 87, 194, 330, 762, 4009, 876,
                                             379, 151, 74, 78, 0, 49)]]
        cases += [[(2009, 7, source_long), (6760, 79, 144, 280, 664, 3350, 707,
                                            375, 147, 83, 23, 33, 41)]]
        cases += [[(2010, 52, source_long), (7170, 68, 111, 278, 558, 3061, 766,
                                             356, 144, 68, 17, 44, 45)]]
...
def wide_to_long(source_wide, additional_level, dct):
    ...
    multi_index = pd.MultiIndex.from_product([caseid, additional_level],
                                             names=['Identifier', 'Survey Year'])
