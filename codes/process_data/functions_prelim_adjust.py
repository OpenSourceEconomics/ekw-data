...
def extend_survey_year(agent):
    ...
    agent.loc[cond, 'SURVEY_YEAR'] = (agent['aux_extend_survey_year'] + agent['AGE']).astype('Int64')
...
def simple_two_grade_jump(agent):
    """This function smoothes the HGC variable in cases in which two grades are completed in one year and none in the
    next year so that one grade is completed in each of the two years. E. g. an HGC variable progression of
    12 -> 14 -> 14 is changed to 12 -> 13 -> 14.
    """
    cond_1 = (agent['REAL_HIGHEST_GRADE_COMPLETED'].diff() == 2)
    cond_2 = (agent['REAL_HIGHEST_GRADE_COMPLETED'] == agent['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1))
    cond_3 = ((agent['MONTHS_ATTENDED_SCHOOL'].shift(1) > 0) & (agent['MONTHS_ATTENDED_SCHOOL'] > 0))
...
def months_attended_school(agent):
    """This function calculates the amount of months in which an individual attended school in a given year.
    """
...
def data_shift(agent):
    ...
    is translated to the occupation code from the job number (last two digits of the labor force status)
    in the survey round (first one or two digits of the labor force status)
    """
    ...
    for week_num in [1, 7, 13, 14, 20, 26, 40, 46, 52]:
        for num in [1, 2, 3, 4, 5]:
            ...
            # Create a matrix of size k x k ((k = # of survey years) in which both rows and columns correspond
            # to the survey years. This matrix, for a given week and job number, indicates
            # to which column year the labor force status in a row year corresponds
            ...
            # Fill the columns (with years as column headers) of the additionally created matrices with
            # the occupation code corresponding to the job number in the year given by the column header.
            ...
            # use the matrix 'cond' to pick only those elements from the matrices created in the previous two steps
            # for which 'cond' evaluates to 'True', i. e. only elements for which the occupation code for a week
            # (indicated by 'week_num') in the row year corresponds to the jobs reported in the column year.

            # As there is only one non-missing value in each row (the occupation code for the job number and week
            # in the row year), we can sum up each row across all columns, again receiving the suitable occupation code.

            # As a result, we receive a vector of size k x 1 (k = # of survey years) in which the value of each row
            # equals the occupation code in the row year for the week number given by 'week_num' and the job number
            # given by 'num'.
            ...
            agent_ext.drop(['JOB_' + repr(int(num)) + '_WK_' + repr(week_num)], axis=1, inplace=True)
            agent_ext.drop(['WAGE_' + repr(int(num)) + '_WK_' + repr(week_num)], axis=1, inplace=True)
