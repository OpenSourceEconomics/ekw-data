...
def weeks_hours_worked(agent):
    """This function creates the variables that are necessary to check the requirements for the
    work choices, i. e. it creates a variable for the number of weeks in a year (of the 9
    representative weeks) that an individual worked, the number of weeks for which the
    EMP_STATUS variable was not missing, i.e. not equal to 0, and a variable for the
    ...
    # number of weekly hours worked which are necessary to determine the 'average hours
    # worked' criterion (subtract weeks in military as no information on weekly hours worked is
    # available for them)
    agent["NECESSARY_WEEKS_WORKED"] = (agent["EMP_NONMISSING_WEEKS"] * (2 / 3)).apply(
        lambda x: math.ceil(x)
    ) - agent["WORKED_WEEKS_MILITARY"]

    # initialize a list of the n highest values of "hours worked per week" in a given year where
    # n is equal to the value of 'NECESSARY_WEEKS_WORKED'
...
def create_wages(agent):
    """This function creates real and nominal incomes for a given individual working in either a
    blue-collar or white-collar occupation following the instructions on page 484 of KW97
    """
...
def create_military_wages(agent):
    ...
    agent["CALCULATED_MILITARY_INCOME"] = (
        agent["sum_military_income"] / agent["weeks_in_military"] * 50
    )
