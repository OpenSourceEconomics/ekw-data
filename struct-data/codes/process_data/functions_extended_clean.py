...
def get_occ_hours(agent):
    ...
    for occ in ['white_collar', 'blue_collar']:
        ...
        for week_num in [1, 7, 13, 14, 20, 26]:
            ...
            agent[occ.upper() + '_HOURS'] += agent['EMP_HOURS_WK_' + repr(week_num)].shift(-1).fillna(0).mul(cond)

        for week_num in [40, 46, 52]:
            cond = (agent['OCC_JC_WK_' + repr(week_num)] == occ).astype(int).fillna(0)
            agent[occ.upper() + '_HOURS'] += agent['EMP_HOURS_WK_' + repr(week_num)].fillna(0).mul(cond)

    return agent


def weeks_hours_worked(agent):
    """This function creates the variables that are necessary to check the requirements for the work choices, i. e.
    it creates a variable for the number of weeks in a year (of the 9 representative weeks) that an individual worked,
    the number of weeks for which the EMP_STATUS variable was not missing, i.e. not equal to 0, and a variable for the
    ...
    for week_num in [40, 46, 52]:
        ...
        cond_3 = agent['EMP_STATUS_WK_' + repr(week_num)].ne(0) & ~agent['EMP_STATUS_WK_' + repr(week_num)].isna()
        agent['EMP_NONMISSING_WEEKS'] += cond_3.astype(int)

        agent['HOURS_LIST'] = (agent['HOURS_LIST'] + agent['EMP_HOURS_WK_' + repr(week_num)].fillna(0).mul(cond_1)
                               .apply(lambda x: [x]))
    ...
    for week_num in [1, 7, 13, 14, 20, 26]:
        ...
        cond_3 = agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).ne(0) & \
            ~agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).isna()
        agent['EMP_NONMISSING_WEEKS'] += cond_3.astype(int)

        agent['HOURS_LIST'] = (agent['HOURS_LIST'] + agent['EMP_HOURS_WK_' + repr(week_num)].shift(-1).fillna(0)
                               .mul(cond_1).apply(lambda x: [x]))

    agent['WORKED_HOURS'] = agent['HOURS_LIST'].apply(lambda x: sum(x))

    # number of weekly hours worked which are necessary to determine the 'average hours worked' criterion (subtract
    # weeks in military as no information on weekly hours worked is available for them)
    agent['NECESSARY_WEEKS_WORKED'] = ((agent['EMP_NONMISSING_WEEKS'] * (2 / 3)).apply
                                       (lambda x: math.ceil(x)) - agent['WORKED_WEEKS_MILITARY'])

    # initialize a list of the n highest values of "hours worked per week" in a given year where n is equal
    # to the value of 'NECESSARY_WEEKS_WORKED'
    agent['MAX_HOURS'] = np.nan

    for num in [1, 2, 3, 4, 5, 6]:
        cond = agent['NECESSARY_WEEKS_WORKED'].eq(num)
        agent.loc[cond, 'MAX_HOURS'] = agent.loc[cond, 'HOURS_LIST'].apply(lambda x: nlargest(num, x))
...
def clean_missing_wages(agent):
    ...
    for week_num in [1, 7, 13, 14, 20, 26]:
        cond = (agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).ge(100) &
                agent['WAGE_WK_' + repr(week_num)].shift(-1).isna())
        agent['MISSPECIFIED_EMP_STATUS'] += cond.astype(int)

    for week_num in [40, 46, 52]:
        cond = agent['EMP_STATUS_WK_' + repr(week_num)].ge(100) & agent['WAGE_WK_' + repr(week_num)].isna()
        agent['MISSPECIFIED_EMP_STATUS'] += cond.astype(int)
...
def clean_military_wages(agent):
    ...
    for week_num in range(1, 40):
        cond = agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).eq(7.0) & agent['INCOME_MILITARY'].shift(-1).isna()
        agent['MILITARY_INCOME_NA'] += cond.astype(int)
...
def create_wages(agent):
    """This function creates real and nominal incomes for a given individual working in either a blue-collar or
    white-collar occupation following the instructions on page 484 of KW97
    """
    ...
    for week_num in [1, 7, 13, 14, 20, 26]:
        ...
        nominal_weekly_wage = agent['WAGE_WK_' + repr(week_num)].shift(-1) * agent[
            'EMP_HOURS_WK_' + repr(week_num)].shift(-1)
        agent['NOMINAL_SUM_WEEKLY_WAGES'] += (nominal_weekly_wage * cond).fillna(0)
        agent['REAL_SUM_WEEKLY_WAGES'] += ((nominal_weekly_wage / agent['GNP_DEFL_BASE_1987'].shift(-1)) * cond).fillna(
            0)
        agent['WEEKS_IN_MAJOR_OCC'] += cond
        agent['HOURLY_WAGES'] += agent['WAGE_WK_' + repr(week_num)].shift(-1) / agent['GNP_DEFL_BASE_1987'].shift(-1)

    for week_num in [40, 46, 52]:
        cond = (agent['OCC_JC_WK_' + repr(week_num)] == agent['CHOICE']).astype(int)
        nominal_weekly_wage = agent['WAGE_WK_' + repr(week_num)] * agent['EMP_HOURS_WK_' + repr(week_num)]
        agent['NOMINAL_SUM_WEEKLY_WAGES'] += (nominal_weekly_wage * cond).fillna(0)
        agent['REAL_SUM_WEEKLY_WAGES'] += ((nominal_weekly_wage / agent['GNP_DEFL_BASE_1987']) * cond).fillna(0)
        agent['WEEKS_IN_MAJOR_OCC'] += cond
        agent['HOURLY_WAGES'] += agent['WAGE_WK_' + repr(week_num)] / agent['GNP_DEFL_BASE_1987']

    agent['REAL_AVG_WEEKLY_WAGE'] = agent['REAL_SUM_WEEKLY_WAGES'] / (agent['WEEKS_IN_MAJOR_OCC'] * 100)
    agent['NOMINAL_AVG_WEEKLY_WAGE'] = agent['NOMINAL_SUM_WEEKLY_WAGES'] / (agent['WEEKS_IN_MAJOR_OCC'] * 100)
    ...
    agent.drop(columns=['REAL_SUM_WEEKLY_WAGES', 'NOMINAL_SUM_WEEKLY_WAGES', 'REAL_AVG_WEEKLY_WAGE', 'NOMINAL_AVG_WEEKLY_WAGE',
                'WEEKS_IN_MAJOR_OCC', 'HOURLY_WAGES'], inplace=True)

    return pd.DataFrame(agent)


def create_military_wages(agent):
    """This function creates real and nominal incomes in the military for a given individual following the instructions
    on page 484 of KW97. As there is no information on hours worked when an individual is in the military, average hours
    worked per week are assumed to be constant across weeks.
    """
    ...
    for week_num in range(1, 53):
        agent['WEEKS_MILITARY_PER_CALENDAR_YEAR'] += (agent['EMP_STATUS_WK_' + repr(week_num)].eq(7.0)).astype('int')

    agent['WEEKLY_MILITARY_INCOME'] = agent['INCOME_MILITARY'] / agent['WEEKS_MILITARY_PER_CALENDAR_YEAR']

    for week_num in range(1, 40):
        cond = (agent['EMP_STATUS_WK_' + repr(week_num)].shift(-1).eq(7.0)).astype(int)
        agent['sum_military_income'] += (agent['WEEKLY_MILITARY_INCOME'].shift(-1) / agent['GNP_DEFL_BASE_1987'].shift(
            -1)).fillna(0) * cond
        agent['weeks_in_military'] += cond

    for week_num in range(40, 53):
        cond = (agent['EMP_STATUS_WK_' + repr(week_num)].eq(7.0)).astype(int)
        agent['sum_military_income'] += (agent['WEEKLY_MILITARY_INCOME'] / agent['GNP_DEFL_BASE_1987']).fillna(0) * cond
        agent['weeks_in_military'] += cond

    agent['CALCULATED_MILITARY_INCOME'] = agent['sum_military_income'] / agent['weeks_in_military'] * 50
