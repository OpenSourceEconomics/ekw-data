...
df['GRADE_COMPLETED'] = df['REAL_HIGHEST_GRADE_COMPLETED'].groupby(df['IDENTIFIER']).diff().shift(-1)
...
school_cond_1 = (df['ENROLLED_SCHOOL_APRIL'].shift(-1).eq(1.0) |
...
df['SCHOOL_NONMISSING_MONTHS'] = ((~(df['ENROLLED_SCHOOL_OCTOBER'].isna())).astype(int) +
...
# previous data
gnp_deflation = pd.read_csv('../../sources/gnp_deflator_data/st_louis_fed_deflator.csv', index_col='SURVEY_YEAR')

gnp_deflation['GNP_DEFL_BASE_1987'] = gnp_deflation['IMPLICIT_GNP'] / gnp_deflation.loc[1987, 'IMPLICIT_GNP']
...
categories_df.columns = ['OCC1990', 'DESCRIPTION', 'CPS_1970', 'CPS_2000_1', 'CPS_2000_5', 'category_service',
                         'CATEGORY', 'CPS_2002']
...
category_dict_1970 = dict(zip(categories_df.CPS_1970[cond_1].astype('Int64'), categories_df.CATEGORY[cond_1]))

cond_2 = categories_df.CPS_2000_1.isin(range(1, 1000))
category_dict_2000 = dict(zip(categories_df.CPS_2000_1[cond_2].astype('Int64'), categories_df.CATEGORY[cond_2]))

cond_3 = categories_df.CPS_2002.isin(range(1, 10000))
category_dict_2002 = dict(zip(categories_df.CPS_2002[cond_3].astype('Int64'), categories_df.CATEGORY[cond_3]))
...
for week_num in [1, 7, 13, 14, 20, 26, 40, 46, 52]:
    ...
    df.loc[cond_1, 'OCC_JC_WK_' + repr(week_num)] = df.loc[cond_1, 'JOB_CHOICE_WK_' + repr(week_num)].astype(
        'Int64').map(category_dict_1970)
    ...
    df.loc[cond_2, 'OCC_JC_WK_' + repr(week_num)] = df.loc[cond_2, 'JOB_CHOICE_WK_' + repr(week_num)].astype(
        'Int64').map(category_dict_2000)
    ...
    df.loc[cond_3, 'OCC_JC_WK_' + repr(week_num)] = df.loc[cond_3, 'JOB_CHOICE_WK_' + repr(week_num)].astype(
        'Int64').map(category_dict_2002)
...
for week_num in [1, 7, 13, 14, 20, 26]:
    df['WHITE_COLLAR'] += (df['OCC_JC_WK_' + repr(week_num)] == 'white_collar').astype(int).fillna(0)
    ...
    df['OUT_OF_LABOR_FORCE'] += (df['EMP_STATUS_WK_' + repr(week_num)].eq(5.0)).astype(int).fillna(0)
...
for choice in ['WHITE_COLLAR', 'BLUE_COLLAR', 'MILITARY', 'UNEMPLOYED',
               'OUT_OF_LABOR_FORCE', 'NOT_WORKING']:
...
df.loc[cond_1 & cond_2, 'CHOICE'] = (df.loc[cond_1 & cond_2, ['WHITE_COLLAR', 'BLUE_COLLAR', 'MILITARY']].
                                     idxmax(axis=1).astype(str).str.lower())
...
cond = (df['CHOICE'].eq('home') &
        ((df['UNEMPLOYED'] + df['OUT_OF_LABOR_FORCE'] + df['NOT_WORKING']) / df['EMP_NONMISSING_WEEKS']).gt(1 / 3))
...
# 'part_time_work'
cond = df['CHOICE'].eq('home')
df.loc[cond & work_cond_1 & ~work_cond_2 & work_cond_3 & work_cond_4, 'HOME_CHOICE'] = 'part_time_work'
...
cond = df['FAMILY_INCOME'].gt((0.5 * df['FAMILY_INCOME'].median())) & df['FAMILY_INCOME'].le(df['FAMILY_INCOME'].median())
df.loc[cond, 'PARENTAL_INCOME_LOW'] = 1

df['PARENTAL_INCOME_HIGH'] = 0
cond = df['FAMILY_INCOME'].ge(df['FAMILY_INCOME'].median()) & df['FAMILY_INCOME'].le((2 * df['FAMILY_INCOME'].median()))
...
cond = (df['AGE'] == 16) & (df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1) >= 10) & (df['SURVEY_YEAR'] == 1977)
...
cond = (df['AGE'] == 16) & (df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1) == 9) & (df['SURVEY_YEAR'] == 1977) & (
        df['CHOICE'].shift(-1) == 'schooling')
df.loc[cond, 'CHOICE'] = df.loc[cond, 'SPECIFIC_CHOICE'] = 'schooling'
df.loc[cond, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[cond, 'aux_schooling'] - 1

cond = (df['AGE'] == 16) & (df['SURVEY_YEAR'] == 1977) & (df['CHOICE'] != 'schooling')
...
df.loc[cond_1 & cond_2, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[cond_1 & cond_2, 'aux_schooling'] - 1
...
cond = (df['REAL_HIGHEST_GRADE_COMPLETED'].isna() & df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1).isna()
        & df['IDENTIFIER'].eq(df['IDENTIFIER'].shift(-1)))
df.loc[cond, 'CONSECUTIVE_HIGHEST_GRADES_MISSING'] = 1

df['CONSECUTIVE_HIGHEST_GRADES_MISSING'] = df.groupby('IDENTIFIER').CONSECUTIVE_HIGHEST_GRADES_MISSING.cumsum()
