...
df = pd.merge(df, age_df, how='outer', left_on=['IDENTIFIER', 'AGE_OCT'], right_on=['IDENTIFIER', 'AGE'])
...
# Gaps in the HGC variables and the 'highest grade attended' cannot decrease and gaps in these variables can thus be
# potentially interpolated. We interpolate these variables if the values before and after the gaps are the same (i.
# e. simply fill gaps with this value) or if the values before and after the gaps suggest that one grade level was
# completed in each of the years with missing values (i. e. fill gaps linearly, increasing by one each year)
for prefix in ['REVISED_', 'UNREVISED_']:
    df[prefix + 'AUX_HGC'] = df[prefix + 'HIGHEST_GRADE_COMPLETED_MAY'].copy()

    df[prefix + 'HIGHEST_GRADE_COMPLETED_MAY'] = df[prefix + 'HIGHEST_GRADE_COMPLETED_MAY'].groupby(
        df['IDENTIFIER']).apply(lambda x: x.interpolate(method='linear', limit_area='inside'))
...
df['HIGHEST_GRADE_ATTENDED'] = df['HIGHEST_GRADE_ATTENDED'].groupby(df['IDENTIFIER']).apply(
...
# As the HGC variables indicate the highest grade completed in May of each year, the HGC variable and the enrollment
# status variable have to be shifted by one given the definition of a year in KW97

df['REAL_HIGHEST_GRADE_COMPLETED'] = df['REVISED_HIGHEST_GRADE_COMPLETED_MAY'].groupby(df['IDENTIFIER']).shift(-1)
df['REAL_ENROLLMENT_STATUS'] = df['REVISED_ENROLLMENT_STATUS_MAY'].groupby(df['IDENTIFIER']).shift(-1)
...
# Changes to 'JOB_1' if the values in 'JOB_1' are not valid 1970 occupation codes and are equal to CPSOCC70. The
# values are corrected with help of the CPSOCC80 variable in which the CPS job of the year is coded in 1980
# occupation codes
...
# 'REAL_HIGHEST_GRADE_COMPLETED' as of 10/1978(AGE 17): 11 BUT: received GED in 02/1979 and therefore seems to have
# not completed grade 12 as 'REAL_HIGHEST_GRADE_COMPLETED' at age 17 equals 11, REAL_HIGHEST_GRADE_COMPLETED' at age
...
# Identifier: 5704
# HIGHEST_GRADE_ATTENDED: 11, last month/year enrolled in school: 03/1978 => highest grade completed in 10/1977: 10
# => 'REAL_HIGHEST_GRADE_COMPLETED' at age 16 (1977): 10, consistent with KW data
cond = (df['IDENTIFIER'] == 5704) & (df['AGE'] == 16)
df.loc[cond, 'REAL_HIGHEST_GRADE_COMPLETED'] = 10


# For a small number of individuals, there are decreases in the HGC variable from one year to the next, e. g. from
# 16 to 15 in the following year. As those decreases are not actually possible, they are corrected on the basis of
...
cond_2 = ~(df['REAL_HIGHEST_GRADE_COMPLETED'].shift(1).isna()) & (df['REAL_HIGHEST_GRADE_COMPLETED'].isna()) & ~(
    df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1).isna())
cond_3 = ((df['REAL_HIGHEST_GRADE_COMPLETED'].shift(1) + 1) == df['REAL_HIGHEST_GRADE_COMPLETED'].shift(-1))

df.loc[cond_1 & cond_2 & cond_3, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[cond_1 & cond_2, 'AUX_HGC'] + 1
df.loc[~cond_1 & cond_2 & cond_3, 'REAL_HIGHEST_GRADE_COMPLETED'] = df.loc[~cond_1 & cond_2, 'AUX_HGC']

df.drop(columns=['AUX_HGC'])

df['AUX_HGC'] = df['REAL_HIGHEST_GRADE_COMPLETED'].copy()

df['REAL_HIGHEST_GRADE_COMPLETED'] = df['REAL_HIGHEST_GRADE_COMPLETED'].groupby(df['IDENTIFIER']).apply(
    lambda x: x.interpolate(method='linear', limit_area='inside'))
